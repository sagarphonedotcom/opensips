...
int run_compare_rec(void *e_data, void *data, void *r_data)
{
	...
	if ((old_rec->state == REGISTERED_STATE) &&
	    (str_strcmp(&old_rec->td.rem_uri, &new_rec->td.rem_uri) == 0) && (str_strcmp(&old_rec->contact_uri, &new_rec->contact_uri) == 0) && (str_strcmp(&old_rec->server_expiry, &new_rec->server_expiry) == 0)) {
